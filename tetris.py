...
def create_grid(locked_pos={}):
    """This creates an empty grid . Every column(size = 10) and every row(size = 20) is set to black color(0, 0, 0)"""
    grid = [[(0, 0, 0) for x in range(10)] for x in range(20)]

    # if an shape is in a locked position, set the grid to the color of the shape
...
def convert_shape_format(piece):
    positions = []
    piece_shape = piece.shape[piece.rotation % len(piece.shape)]
    print(piece_shape)
    for i, line in enumerate(piece_shape):
        row = list(line)
...
def draw_next_shape(shape, surface):
    ...
    piece_shape = shape.shape[shape.rotation % len(shape.shape)]
...
def draw_window(surface, grid, score=0):
    ...
    score_pos_x = top_left_x + play_width + 60
    score_pos_y = top_left_y + play_height/2 - 200
    score_text = 'Score: ' + str(score)
    score_label = score_font.render(score_text, 1, (255, 255, 255))

    surface.blit(score_label, (score_pos_x, score_pos_y))
...
def main_menu(win):
    ...
    while run:
        win.fill((0, 0, 0))
        draw_text_middle('Press key to play', 60, (255, 255, 255), win)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
